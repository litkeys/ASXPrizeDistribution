...
max_deviation = sum(value - lowest_portfolio for value in member_portfolios)

deviation_ratios = [(value - lowest_portfolio) / max_deviation for value in member_portfolios]

bonus_prizes = [minimum_prize + deviation_ratios[i] * bonus_pool for i in range(member_count)]

print("Below are the prizes (from low to high)")
for prize in bonus_prizes:
    print(round(prize, 1))